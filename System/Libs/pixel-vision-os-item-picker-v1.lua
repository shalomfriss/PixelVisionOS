...
function PixelVisionOS:UpdateItemPicker(data)
  ...
  if(data.picker.inFocus == true) then
    ...
    if(data.dragging == true) then
      ...
      if(self.editorUI.collisionManager:MouseInRect(data.visibleRect) and overPos.index < data.total) then

        -- overPos = self:CalculateItemPickerPosition(data)
        overPos.x = data.visibleRect.x + (overPos.x * data.itemSize.x) - data.lastStartX
        overPos.y = data.visibleRect.y + (overPos.y * data.itemSize.y) - data.lastStartY
      else
        overPos.x = self.editorUI.collisionManager.mousePos.x
        overPos.y = self.editorUI.collisionManager.mousePos.y

        -- Offset the item so you can see it under the cursor
        local offset = (data.scale * 8) * .5

        -- Apply offset
        overPos.x = overPos.x - offset
        overPos.y = overPos.y - offset
      ...
      if(overPos.index == -1) then
        overPos = self.editorUI.collisionManager.mousePos
      end

      overPos.x = data.rect.x + (overPos.x * data.itemSize.x) - data.lastStartX
      overPos.y = data.rect.y + (overPos.y * data.itemSize.y) - data.lastStartY
    end

    -- test to invalidate over sprite data
    local invalidPixelData = data.lastOverID ~= overPos.index
    ...
    if(data.lastOverID ~= overPos.index) then
      ...
      local tmpX = data.pressSelection == nil and overPos.x - data.rect.x + data.lastStartX or data.pressSelection.x * data.itemSize.x
      local tmpY = data.pressSelection == nil and overPos.y - data.rect.y + data.lastStartY or data.pressSelection.y * data.itemSize.y

      data.lastOverID = overPos.index
    ...
    data.overItemDrawArgs[2] = overPos.x
    data.overItemDrawArgs[3] = overPos.y
    ...
    if(data.dragging) then
      ...
      if(overPos.index ~= nil and overPos.index ~= -1 and overPos.index < data.picker.total) then
        data.picker.toolTip = "Swap "..data.toolTipLabel.." ID " .. string.lpad(tostring(data.pressSelection.index), data.totalItemStringPadding, "0") .. " with ".. string.lpad(tostring(overPos.index), data.totalItemStringPadding, "0")
      else
        data.picker.toolTip = "Dragging "..data.toolTipLabel.." ID " .. string.lpad(tostring(data.pressSelection.index), data.totalItemStringPadding, "0")
      end

    elseif(overPos.index ~= nil and overPos.index ~= -1) then

      -- Update the tooltip with the index and position
      data.picker.toolTip = "Select "..data.toolTipLabel.." ID " .. string.lpad(tostring(overPos.index), data.totalItemStringPadding, "0")
