...
function Init()
    ...
    if(success == true) then
        ...
        soundIDStepper = editorUI:CreateNumberStepper({x = 32, y = 16}, 16, 0, 0, totalSounds - 1, "top", "Sound ID value.")

        soundIDStepper.onInputAction = OnChangeSoundID

        songNameFieldData = editorUI:CreateInputField({x = 88, y = 24, w = 80}, "Untitled", "Change the label of the selected sound.", "name")
        songNameFieldData.onAction = OnChangeName

        channelIDStepper = editorUI:CreateNumberStepper({x = 176, y = 16}, 8, - 1, 0, tostring(gameEditor:TotalChannels()) - 1, "top", "The channel sound effects will be previewed on.")

        channelIDStepper.onInputAction = OnChangeChannelID

        waveInputField = editorUI:CreateInputField({x = 224, y = 24, w = 16}, validWaves[1], "Wave type.", nil, nil)
        ...
        for i = 1, totalKnobs do
            ...
            data.knobUI = editorUI:CreateKnob({x = data.x, y = data.y, w = 24, h = 24}, "knob", "Change the volume.", PaletteOffset(7))
        ...
        for i = 1, totalSFXButtons do
            ...
            data.buttonUI = editorUI:CreatePaletteButton({x = data.x, y = data.y}, data.spriteName, data.toolTip, PaletteOffset( 1))
